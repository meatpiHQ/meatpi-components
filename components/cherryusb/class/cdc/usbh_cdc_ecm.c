...
int usbh_cdc_ecm_eth_output(uint32_t buflen)
{
     ...
     * Mirror of the usbh_cdc_ncm fix. */
    if (ret >= 0 && buflen != 0 &&
        (buflen % USB_GET_MAXPACKETSIZE(g_cdc_ecm_class.bulkout->wMaxPacketSize)) == 0) {
        usbh_bulk_urb_fill(&g_cdc_ecm_class.bulkout_urb, g_cdc_ecm_class.hport, g_cdc_ecm_class.bulkout, g_cdc_ecm_tx_buffer, 0, USB_OSAL_WAITING_FOREVER, NULL, NULL);
