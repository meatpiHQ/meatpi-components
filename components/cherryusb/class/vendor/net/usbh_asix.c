...
int usbh_asix_eth_output(uint32_t buflen)
{
     ...
     * driver's equivalent of the ZLP rule). Same silent-drop class as
     * BUG_NCM_HOST_TX_ZLP.md. */
    if (((buflen + 4) % USB_GET_MAXPACKETSIZE(g_asix_class.bulkout->wMaxPacketSize)) == 0) {
        USB_LOG_DBG("txlen:%d\r\n", buflen + 8);
