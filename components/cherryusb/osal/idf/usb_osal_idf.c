...
usb_osal_thread_t usb_osal_thread_create(const char *name, uint32_t stack_size, uint32_t prio, usb_thread_entry_t entry, void *args)
{
    ...
    /* WiCAN (internal-RAM budget): PSRAM stacks for every CherryUSB
     * thread (hub/psc + eth-class RX). These threads never touch
     * flash, URBs/class state are internal .bss (the IRAM ISR never
     * reads a PSRAM stack), and vTaskDeleteWithCaps handles the
     * self-delete pattern (usb_osal_thread_delete(NULL)). TCB stays
     * internal (WithCaps allocates it with pvPortMalloc). */
    xTaskCreatePinnedToCoreWithCaps(entry, name, stack_size, args,
                                    configMAX_PRIORITIES - 1 - prio,
                                    &htask,
...
{
    /* WiCAN: WithCaps tasks must be deleted with the WithCaps API
     * (frees the PSRAM stack + internal TCB; NULL = self-delete). */
