...
static void *esp_isotp_calloc(size_t count, size_t size, bool prefer_psram)
{
#ifdef ESP_PLATFORM
    if (prefer_psram) {
...
typedef struct esp_isotp_link_t {
    ...
    bool externally_managed_twai;             ///< True when caller owns TWAI callbacks/lifecycle
    bool tx_use_padding;                      ///< True to pad generated frames to 8 bytes
    uint8_t tx_padding_value;                 ///< Byte value used for transmit padding
    esp_isotp_can_tx_callback_t can_tx_callback;   ///< Optional externally managed CAN TX callback
    void *can_tx_callback_arg;                ///< User argument for can_tx_callback
} esp_isotp_link_t;

static esp_err_t esp_isotp_process_rx_frame(esp_isotp_handle_t handle, const twai_frame_t *frame)
{
    ESP_RETURN_ON_FALSE(handle && frame && frame->buffer, ESP_ERR_INVALID_ARG, TAG, "Invalid parameters");
    ESP_RETURN_ON_FALSE(frame->buffer_len <= sizeof(handle->isr_rx_frame_buffer.data_payload),
                        ESP_ERR_INVALID_ARG, TAG, "Invalid TWAI frame size");

    if (frame->header.id != handle->link.receive_arbitration_id) {
        return ESP_OK;
    }

    isotp_on_can_message(&handle->link, frame->buffer, frame->buffer_len);
    return ESP_OK;
}
...
static IRAM_ATTR bool esp_isotp_rx_callback(twai_node_handle_t handle, const twai_rx_done_event_data_t *edata, void *user_ctx)
{
    ...
    (void)esp_isotp_process_rx_frame(link_handle, &rx_frame->frame);
...
int isotp_user_send_can(const uint32_t arbitration_id, const uint8_t *data, const uint8_t size, void *user_data)
{
    ...
    if (isotp_handle->tx_use_padding && size < sizeof(padded_data)) {
        memcpy(padded_data, data, size);
        memset(padded_data + size,
               isotp_handle->tx_padding_value,
               sizeof(padded_data) - size);
        tx_data = padded_data;
        tx_size = sizeof(padded_data);
    }

    if (isotp_handle->can_tx_callback) {
        esp_err_t callback_ret = isotp_handle->can_tx_callback(arbitration_id,
                                                               tx_data,
                                                               tx_size,
                                                               isotp_handle->can_tx_callback_arg);
        if (callback_ret == ESP_OK) {
            return ISOTP_RET_OK;
        }

        return (callback_ret == ESP_ERR_TIMEOUT) ? ISOTP_RET_NOSPACE : ISOTP_RET_ERROR;
    }

    twai_node_handle_t twai_node = isotp_handle->twai_node;
    ESP_RETURN_ON_FALSE_ISR(twai_node != NULL, ISOTP_RET_ERROR, TAG, "Invalid TWAI node");
    ...
    memcpy(tx_frame->data_payload, tx_data, tx_size);

    tx_frame->frame.buffer = tx_frame->data_payload;
    tx_frame->frame.buffer_len = tx_size;
    ...
    if (ret != ESP_OK) {
        ...
        return (ret == ESP_ERR_TIMEOUT) ? ISOTP_RET_NOSPACE : ISOTP_RET_ERROR;
    }
...
esp_err_t esp_isotp_new_transport(twai_node_handle_t twai_node, const esp_isotp_config_t *config, esp_isotp_handle_t *out_handle)
{
    ...
    ESP_RETURN_ON_FALSE(config && out_handle, ESP_ERR_INVALID_ARG, TAG, "Invalid parameters");
    ESP_RETURN_ON_FALSE(twai_node || config->can_tx_callback, ESP_ERR_INVALID_ARG, TAG, "TWAI node or CAN TX callback required");
    ESP_RETURN_ON_FALSE(twai_node || config->externally_managed_twai, ESP_ERR_INVALID_ARG, TAG, "NULL TWAI node requires externally managed mode");
    ...
    isotp->isotp_tx_buffer = esp_isotp_calloc(config->tx_buffer_size, sizeof(uint8_t), true);
    isotp->isotp_rx_buffer = esp_isotp_calloc(config->rx_buffer_size, sizeof(uint8_t), true);
    ...
    isotp->externally_managed_twai = config->externally_managed_twai;
    isotp->tx_use_padding = config->tx_use_padding;
    isotp->tx_padding_value = config->tx_padding_value;
    isotp->can_tx_callback = config->can_tx_callback;
    isotp->can_tx_callback_arg = config->can_tx_callback_arg;

    ESP_LOGI(TAG,
             "new transport: tx_id=0x%lx rx_id=0x%lx tx_buf=%lu rx_buf=%lu frame_pool=%lu external=%d",
             (unsigned long)config->tx_id,
             (unsigned long)config->rx_id,
             (unsigned long)config->tx_buffer_size,
             (unsigned long)config->rx_buffer_size,
             (unsigned long)config->tx_frame_pool_size,
             config->externally_managed_twai);
    ...
    if (!isotp->externally_managed_twai) {
...
esp_err_t esp_isotp_delete(esp_isotp_handle_t handle)
{
    ...
    ESP_LOGI(TAG,
             "delete transport: tx_id=0x%lx rx_id=0x%lx",
             (unsigned long)handle->link.send_arbitration_id,
             (unsigned long)handle->link.receive_arbitration_id);

    if (!handle->externally_managed_twai) {
...
esp_err_t esp_isotp_feed_can_frame(esp_isotp_handle_t handle, const twai_frame_t *frame)
{
    return esp_isotp_process_rx_frame(handle, frame);
}
...
esp_err_t esp_isotp_send(esp_isotp_handle_t handle, const uint8_t *data, uint32_t size)
{
    ...
    ESP_LOGI(TAG,
             "send request: tx_id=0x%lx rx_id=0x%lx size=%lu",
             (unsigned long)handle->link.send_arbitration_id,
             (unsigned long)handle->link.receive_arbitration_id,
             (unsigned long)size);
    ...
    switch (ret) {
    case ISOTP_RET_OK:
        ESP_LOGI(TAG, "send request accepted: size=%lu", (unsigned long)size);
        return ESP_OK;
    case ISOTP_RET_INPROGRESS:
        ESP_LOGI(TAG, "send request in progress: size=%lu", (unsigned long)size);
        return ESP_ERR_NOT_FINISHED;
    case ISOTP_RET_OVERFLOW:    // Buffer overflow
    case ISOTP_RET_NOSPACE:     // Not enough space in internal buffers
        ESP_LOGW(TAG, "send request no mem: size=%lu ret=%d", (unsigned long)size, ret);
        return ESP_ERR_NO_MEM;
    case ISOTP_RET_LENGTH:      // Payload size exceeds buffer size
        ESP_LOGW(TAG, "send request invalid size: size=%lu ret=%d", (unsigned long)size, ret);
        return ESP_ERR_INVALID_SIZE;
    case ISOTP_RET_TIMEOUT:
        ESP_LOGW(TAG, "send request timeout: size=%lu", (unsigned long)size);
