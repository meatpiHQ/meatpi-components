...
#define ML_TASK_WG_MGR_STACK    (7 * 1024) /* WiCAN: was 8K; internal is the scarce pool and the measured watermark leaves >2K headroom (bench 2026-07-07) */
...
typedef struct {
    enum {
        ...
        /* Full-netmap mark-and-sweep (BUG_TS_PHANTOM_PEERS.md): a
...
typedef struct {
    ...
    bool use_tls;                   /* WiCAN: false = plain TCP (Headscale embedded DERP) */
    bool connected;
...
struct microlink_s {
    ...
    bool first_map_pending;

    /* WiCAN: long-poll map stream reassembly (PSRAM, lazy alloc; owned
     * by the coord task). ts2021 stream framing is [4B LE length][JSON]
     * and one message may span multiple Noise/H2 frames. */
    uint8_t *map_stream_buf;
    size_t map_stream_len;

    /* WiCAN: long-poll H2 frame accumulation. An H2 frame can span
     * multiple Noise frames (any MapResponse > ~4KB); only complete H2
     * frames may be parsed, the partial tail must be kept for the next
     * noise_recv. (do_fetch_peers accumulates for the same reason.) */
    uint8_t *h2_acc_buf;
    size_t h2_acc_len;

    /* WiCAN: home region validated against the parsed DERPMap (see
     * ml_effective_derp_region) */
    ...
    char ctrl_host[64];
    /* WiCAN: control-plane TCP port (default 80; custom coordinators
     * like Headscale on http://host:8080 override it via control_url) */
    int  ctrl_port;
