...
| `wifi_ram_profile` | enum full/lean/custom | `lean` (was `full` until 2026-09-22) | v5: trades internal RAM for WiFi throughput at boot. `full` = IDF default buffers. `lean` frees **~14.4 KB internal** (static RX 10→6, static TX 8→4, cache TX 32→16) at **no measured throughput cost** for the device's real traffic (MQTT/HTTP are app-capped below WiFi's ceiling; bench 2026-07-08). `custom` = the three knobs below. Runtime — the counts are `wifi_init_config_t` fields applied at `esp_wifi_init`; reboot to take effect. Use `lean` to run WiFi + BLE together (the internal-RAM cliff). Default flipped to `lean` on 2026-09-22 after the BLE controller's static ACL TX buffers (the fix for silent notification loss) took 3.2 KB more internal RAM and a `full` device hit the cliff within minutes (event-loop posts timing out, tunnel 502s, min free 155 B); lean measured 18.4 KB free at boot with BLE on vs 4.8 KB. Devices with a stored value keep it. |
